...
>
> 🟢 **Esta área é declarada pelo próprio cliente.** O campo *"Departamentos engajados"* do template de transição, na página da NV, lista **`Engenharia de Produto`**.
>
...
⚠ **Não há perfil de Engenharia na NV.** ⚠ **A ficha traz `Definição da Grade`, `Grade de Tamanhos` e `Tabela de Medidas`** — funções de engenharia de produto distribuídas entre os perfis existentes.

🔴 **A integração da NV tem uma ação que nenhum outro cliente tem: `Audit uMode x Linx`.** ⚠ **É conferência entre os dois sistemas, e está 🔴 no perfil `NV - Geral`.**
