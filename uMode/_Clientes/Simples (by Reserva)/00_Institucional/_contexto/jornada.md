...
- 🔴 **Não existe linha deste cliente no `Mapa de Clientes`** — logo, **não há status, ativação, módulos nem atendimento para registrar.**
- **O único vestígio é o portal, criado em 18/10/2024, não varrido.**

## Status atual
**`Inativo`** · `[a preencher]` — **campo vazio na base** · ERP: `[a preencher]`

## Fase atual
🔴 **Indeterminada.** **Não há status porque não há linha na base principal.**

## Marcos da jornada

| Data | Marco | Fonte |
|---|---|---|
| **18/10/2024** | **Portal uMode | Simples Reserva** criado — **único marco documentável deste cliente** | base `Portal do Cliente` |
...
## Próximos passos
1. 🔴 **Decidir se `Simples (by Reserva)` é cliente, marca de cliente ou ruído.**
2. **Se for cliente, criar a linha no `Mapa de Clientes`.**
3. 🔴 **Reconciliar as duas bases de cliente** — e **preencher a relação `Clientes` do `Portal do Cliente`**, hoje vazia nas 32 linhas.
4. **Confirmar a relação com Reserva e Oficina Reserva.**

## Histórico de incidentes / alertas
`[a preencher]` — **nenhum chamado desta conta**.

## Observações
- 🔴 **Não existe no `Mapa de Clientes`** — caso inverso da Loungerie.
- ⚠ **Duas grafias:** `Simples (by Reserva)` no corpus, `Simples Reserva` no portal.
- **Terceira marca do ecossistema Reserva no corpus**, com `Reserva` e `Oficina Reserva`.
- **O `Inativo` no status é marcação por ausência de fonte, não valor lido.**
