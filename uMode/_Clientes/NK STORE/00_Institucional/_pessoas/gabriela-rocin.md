...
`[a preencher]`
### Nome completo
`[a preencher]` — a fonte registra **`gabriela rocin`**
### Nome preferido / como é chamado(a)
**gabriela rocin**
### Email
**`gabriela.rocin@nkstore.com.br`** — e-mail **corporativo**, da tabela de usuários da plataforma.
...
| **Status na base** | **CONVITE PENDENTE desde 12/12/2024** |
| Fonte | tabela de usuários da página do cliente |

**Observação da fonte:** perfil de acesso `NK - Estilo`. 🔴 **`Nome` está `(Não definido)` na fonte** — o nome aqui vem da parte local do e-mail, e **isso é dedução minha, não dado.** 🔴 **`Departamento NK` = `INATIVAR`** — ⚠ **não é departamento, é INSTRUÇÃO OPERACIONAL escrita no campo de área.** Não afirmo que saiu: afirmo que a fonte pede a inativação.
