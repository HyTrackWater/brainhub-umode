---
aliases:
  - "[Nome completo] · Pessoa"
tags:
  - tipo/template
  - casa
---
# [Nome completo] · Pessoa

## Identificação
### Foto
[URL ou referência do arquivo — vazio até upload real, nunca placeholder inventado]
### Nome completo
### Nome preferido / como é chamado(a)
### Email
### Cadeira / cargo atual
### Nível HIC
[jornada de aprendizagem e uso de ferramentas/conhecimento em IA do colaborador — critério de
triagem/nível ainda não definido, ver protocolo-gestao-pessoas.md; campo existe, escala não]
### Área (organizacional)
[uma das 8 Áreas internas de CONTEXT.md, quando mapeável — ver nota em
protocolo-gestao-pessoas.md sobre Cadeira ≠ Área]
### Data de entrada na uMode
### Status na uMode
[Ativo / Inativo]
### Data de saída da uMode
[preencher só se Status = Inativo]

## Papel
### Missão da cadeira
### Responsabilidades principais
### Interfaces
[com quem trabalha / de quem depende]

## Histórico
### Áreas de atuação histórica
### Clientes atuais atendidos
### Clientes atendidos historicamente

## Personificação
[só a própria pessoa preenche, via formulário — nunca inferido de documento]
### Como se descreve
### Personalidade / forma de trabalhar
### O que a diferencia
### Curiosidade / algo pessoal

## Competências
[preenchido só com fonte real citada — currículo/CV, LinkedIn, certificado, ou resposta da
própria pessoa via formulário. Nunca inferido do cargo/cadeira ou de como a pessoa aparece em
outro documento — ver protocolo-gestao-pessoas.md]
### Experiência profissional anterior
### Skills / habilidades técnicas
### Cursos e certificações
### Ferramentas e plataformas que domina

## Governança
### Fonte dos dados documentáveis
### Quem pode alterar este documento
