---
aliases:
  - "Tais Moser · Pessoa"
tags:
  - tipo/pessoa
  - casa
---
# Tais Moser · Pessoa

## Identificação
### Foto
[a preencher]
### Nome completo
Tais Moser
### Nome preferido / como é chamado(a)
[a preencher]
### Email
**`tais.moser@umode.com.br`** — e-mail **corporativo**, da base de pessoas do Notion (`collection://c82a689c…`, a mesma que o campo `Atendimento 2024` do `Mapa de Clientes` aponta), lida em 23/09/2026.

🟢 **É a chave de identidade desta pessoa** (item 252). Tier `T2`.

⚠ `Função` na origem: **Key Account** · `Situação`: **Inativo**.

⚠ **A mesma origem traz telefone, endereço com CEP e data de nascimento — `T0`, nenhum copiado.** Registro que existem e onde.
### Cadeira / cargo atual
⚠ **Não se aplica — a pessoa saiu.** **O cargo que a fonte registra é `Customer Success`, e vale para 07 jun 2024**, não para hoje.
### Nível HIC
[a preencher — critério de triagem/escala ainda não definido]
### Área (organizacional)
[a preencher — a ata traz cargo, não Área das 8 travadas em CONTEXT.md; não presumido]
### Data de entrada na uMode
[a preencher]
### Status na uMode
🔴 **NÃO ESTÁ MAIS NA uMode** — confirmado pelo Vinícius em **22 set 2026**. ⚠ **Data de saída não informada.**
### Data de saída da uMode
[a preencher] — 🔴 **sabe-se que saiu; não se sabe quando.**

## Papel
### Missão da cadeira
[a preencher]
### Responsabilidades principais
Customer Success — conforme a ata de 07 jun 2024. ⚠ **Aparece também como `Thaís` na mesma ata** (*contatos da Marina e da Thaís*) — **grafia não unificada**.
### Interfaces
[a preencher]

## Histórico
### Áreas de atuação histórica
[a preencher]
### Clientes atuais atendidos
⚠ **Não se aplica — a pessoa saiu.**
### Clientes atendidos historicamente
**Luiza Barcelos** — participou da reunião de Warm Up em 07 jun 2024. ⚠ **É o único vínculo que a fonte prova**, não a lista completa.

## Personificação
[só a própria pessoa preenche, via formulário — nunca inferido de documento]
### Como se descreve
[a preencher]
### Personalidade / forma de trabalhar
[a preencher]
### O que a diferencia
[a preencher]
### Curiosidade / algo pessoal
[a preencher]

## Competências
[preenchido só com fonte real citada — currículo/CV, LinkedIn, certificado, ou resposta da própria pessoa via formulário. Nunca inferido do cargo/cadeira — ver protocolo-gestao-pessoas.md]
### Experiência profissional anterior
[a preencher]
### Skills / habilidades técnicas
[a preencher]
### Cursos e certificações
[a preencher]
### Ferramentas e plataformas que domina
[a preencher]

## Governança
### Fonte dos dados documentáveis
Ata da reunião de Warm Up com a Luiza Barcelos, **07 jun 2024** (Notion `21bb3e7e…`) — varredura de 22 set 2026. **Saída confirmada pelo Vinícius na mesma data.**
### Quem pode alterar este documento
Liderança de People. 🔴 **O `Status na uMode` de toda a Casa será revisto de uma vez ao fim da varredura** — ver `AGORA.md` § 7.

## Conexões

> Camada de ligação. **Gerada por `scripts/gera-conexoes.py`.**

**Instância:** `Casa uMode` — pessoa INTERNA. Pessoa da Casa **nunca se duplica dentro do cliente** (`CLAUDE.md`).

**As outras pessoas da Casa:** [índice](_indice.md)

**Institucional da Casa:** [institucional.md](../_contexto/institucional.md)

**O protocolo que governa esta ficha:** [`protocolo-gestao-pessoas.md`](../_protocolos/protocolo-gestao-pessoas.md)
