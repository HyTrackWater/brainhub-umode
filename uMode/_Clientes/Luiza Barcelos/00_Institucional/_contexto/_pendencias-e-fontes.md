...
>
> 🔴 **Este é o único lugar onde se pergunta "o que ainda não sei sobre o Luiza Barcelos?" e
...
|---|--:|---|---|
| `Warm Up Cliente` › `Pessoas` › `Diretores e Representantes Legais` | 4 | linhas de contato de **Luiz Raul Aleixo Barcelos** | 🟢 **resolvido** — o Vinícius copiou e colou em 22 set 2026 |
| `Warm Up Cliente` › `Pessoas` › `Responsável pelo Financeiro` | 2 | e-mail e telefone da Ana Lucia Andrade | 🟢 **resolvido** — copiado e colado |
| `Warm Up Cliente` › `Pessoas` › `Responsáveis pelo Projeto` | 8 | contatos e cargos de Gustavo Sobrinho e Gabriel Jaques da Silva | 🟢 **resolvido** — copiado e colado |
| `Warm Up Cliente` › `Pessoas` › `Responsável Tecnologia` | 3 | contatos e o cargo de **Samuel Correa** | 🟢 **resolvido** — copiado e colado |
...
|--:|---|:-:|---|
| 1 | 🔺 **CORREÇÃO REGISTRADA: eu disse que a conta era quase invisível. É a MAIS bem documentada que abri.** Eu media pela base de chamados, janela de 24 dias. **A página tem 15 pessoas com cargo, discovery de Sales, 8 páginas de regra datadas e um relatório de incidente.** | `T2` | nada — corrigido |
| 2 | 🔴 **O processo está na cabeça de UMA pessoa, e a própria uMode escreveu isso:** *o processo está na cabeça da Marcinha — a missão é tirar as informações da cabeça dela e colocar na ferramenta*. **É risco de pessoa-chave nomeado, e a pessoa é a dona da marca.** | `T2` | — |
| 3 | 🔴 **`6 PRIMEIROS MESES PARA MOSTRAR CREDIBILIDADE DA UMODE`** — em vermelho, no kick off interno de **06/06/2024**. E: *o time da Luiza tem trauma de cronograma e entrega por frustrações passadas com a Linx*. **Hoje são 27 meses.** | `T2` | conferência com o atendimento |
| 4 | 🔴 **DUAS datas de ativação que não batem:** a página diz *Data de Ativação por Vendas — 15/05/24* e o campo da base diz **07/06/2024**. **Um mês de diferença, e ninguém sabe qual é a boa.** | `T2` | conferência |
| 5 | 🔴 **`Relatório de Incidente | Weekly Luiza Barcelos <> uMode — 2025/08/08`.** **É o único registro de incidente com cliente que o corpus conhece, e eu não o abri.** | `T2` | tempo de varredura — prioridade |
| 6 | 🟢 **Escopo contratual EXPLÍCITO, ao contrário da Cambos:** *não faz parte do escopo deste contrato a integração com o sistema LINX ou qualquer outro que não seja o SAFETECH*, e **três relatórios nomeados** — grade, status e acompanhamento. **É o modelo de como escrever escopo.** | `T2` | — |
| 7 | 🔴 **`uDash` é PRODUTO CONTRATADO aqui** — *13 usuários uFlow / 2 usuários uDash*. Na VIX ele aparecia como **perfil de acesso**. **Confirma os dois sentidos da mesma palavra.** | `T2` | desambiguação |
| 8 | 🔴 **Duas listas de times que não batem:** o kick off interno diz *Estilo, PCP, Compras*; a seção `Times Envolvidos` diz **oito**: Diretoria Criativa, Estilo, Desenvolvimento, Produto e Merchandising, Suprimentos, Operações (Cadastro e Precificação), Estratégia/Processos/Projetos e Tecnologia. | `T2` | conferência |
| 9 | 🆕 **`HubSpot` — décima ferramenta**, e é o CRM da uMode: há link direto para o *deal* desta conta. **Não estava no enum `tool`.** | `T2` | — |
| 10 | 🆕 **8 páginas de regra datadas** — `Regra do Campo Linha`, `Regra Família 05/08/25`, `Automações da Aba Etapa & Datas 24/09/2025`, `Regras do WorkFlow 12/11`, `Importação das Listas 24/01`. **É a documentação de regra mais disciplinada da carteira.** | `T2` | tempo de varredura |
| 11 | ⚠ **Dois nomes não batem com o próprio e-mail ou consigo mesmos:** `Gabriel Jaques` × `gabriel.silva@`, e `Eduardo Britto` × `Eduardo Brito` **na mesma página**. **Não escolhi nenhuma.** | `T2` | confirmação |
| 12 | 🔴 **18 blocos da página não abriram por este conector** — estão justamente dentro de `Diretores e Representantes Legais` e `Responsáveis pelo Projeto`. **Há mais pessoas ali do que as 15 que consegui ler.** | `T2` | leitura manual ou outro conector |
| 13 | ⚠ **Receita anual declarada na base.** 🔴 **Não replico o valor aqui** — `T1`, fica no `_contexto/` deste cliente. | `T1` | nada — tratamento |
| 14 | ⚠ **ERP `Safe Tech`** — único cliente da carteira que o usa. | `T2` | — |
...
### 3.1 · Varridas para a carteira inteira — valem para o Luiza Barcelos
