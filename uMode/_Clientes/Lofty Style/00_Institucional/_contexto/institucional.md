# Lofty Style · Institucional

## Identidade
### Quem são
Franqueadora de moda.
Razão Social: Lofty Style Franqueadora Confecção LTDA · CNPJ: 05.548.796/0001-35
### O que fazem
Franqueadora de confecção — opera rede de franquias de moda (60 lojas, conforme Mapa de
Clientes uMode). [Detalhamento de produto/coleção: a preencher]
### Para quem fazem
[a preencher]

## Posicionamento
### Segmento
Moda — São Paulo / SP
### Receita anual
[a preencher]
### Grupo de segmentação uMode
Médios
> ⚠ Fonte "Mapa de Clientes" (Notion/CRM, campo `Tamanho atendimento`) traz `G` — não sei se é
> a mesma escala que "Médios" aqui ou uma classificação diferente (ex.: tamanho de atendimento
> vs. porte do cliente). Não fundido — registrado como observação em aberto.

## Operação uMode
### Status atual
Operação Assistida
> ⚠ Fonte "Mapa de Clientes" (CRM Notion, última edição 04/03/2026) traz `Onboarding` — mais
> antigo que os marcos já registrados em `jornada.md` (que já mostram Operação Assistida em
> jan/2026). Mantido `Operação Assistida` por ser a fonte mais recente/específica; o CRM
> provavelmente está desatualizado. Não alterado sem confirmação.
### Data de ativação
10/02/2025
### Módulos contratados
- Gestão de Coleção · Integração · Relatórios · Cronograma
- Confirmado via CRM: produto uFlow · módulos "Gestão de Coleção" e "Gestão de Processos" ·
  12 usuários internos contratados · 4 relatórios com 4 indicadores cada
### ERP / Integração
Linx
### Responsável de atendimento (uMode)
Laura Delgado Cardoso (Key Account) · Andrea Goulart Holmer dos Santos (Key Account) ·
Vanessa Rinaldi Ornelas Engman (Consultora de Negócios)
> Time de atendimento atual conforme CRM ("Holmer & Laura") e confirmado pelos registros de
> reunião — houve um time anterior (Marina Santoro, PO Integração, conduzindo onboarding) até
> ~set/2025, quando o atendimento passou para Andrea Holmer + Laura Delgado. Ver `jornada.md`.

## Aliases de áreas
### Mapeamento alias → canônico
[a preencher — Drive de operação (link abaixo) ainda não foi lido em detalhe]

## Sistemas e fontes de verdade
### Drive de operação
https://drive.google.com/drive/folders/1sP7YqvkGtyoCXkk6BUZwaUzIBaK9JHXV

## Contexto crítico
- CRM "Mapa de Clientes" (Notion, via Drive): Endereço Av. Casa Verde, 463, Casa Verde, CEP
  02519-000, São Paulo/SP · Email financeiro: financeiro@loftystyle.com.br · OKRs: planilha
  própria do cliente (link não copiado aqui, ver CRM) · Documentação: páginas Notion "Plano de
  Sucesso do Cliente - OKRs", "Dores e Soluções", "Warm Up e Kick Off" (não lidas ainda)
- Nenhum registro de feedback/satisfação encontrado na base "Feedback Interno Clientes" da
  Casa (outros clientes têm desde 2024) — Lofty nunca foi avaliado nesse sistema. Gap real,
  não lacuna de leitura.

## Governança
### Responsável de atendimento (uMode)
Laura Delgado Cardoso · Andrea Goulart Holmer dos Santos
### Quem pode alterar este documento
Responsável de atendimento + liderança de Atendimento uMode
