---
tipo: registro
origem: google-meet · resumo do Gemini + transcrição
acervo: acervo Laura Cardoso
titulo: "Weekly Luiza Barcelos · uMode (novo)"
data: 2025-06-20
referente_a: 2025-06-20
destino: luiza-barcelos
confianca_destino: media
natureza: "[a preencher]"
participantes: []
participantes_sem_email:
  - "Laura Delgado"
  - "Gabriel"
  - "Michelle Rodrigues"
tem_transcricao: true
transcricao_automatica: false
tem_resumo: true
posicao_na_linha_do_tempo: 19/34
reunioes_depois: 15
idade_em_dias: 462
horizonte: historico
tier: T2
tier_motivo: "sem dado pessoal nem valor comercial detectado"
processado_em: 2026-09-25
---
# Weekly Luiza Barcelos · uMode (novo) — 2025-06-20

> **Classe: `REGISTRO`.** Evidência datada. 🔴 **Não é autoridade e não se edita.**
> **Fonte:** resumo do Gemini, acervo Laura Cardoso. 🟢 **Esta reunião TEM transcrição de fala** no
> `.docx` original — **6 falas, 3 falantes**. As propostas abaixo vêm do **resumo**
> (derivado); **cada uma com minuto é conferível contra a fala**. 🔴 A transcrição
> bruta não entra no repositório.
> ⚠ **Destino `luiza-barcelos` saiu só do título:** o título mente — pode ser reunião interna *sobre* o cliente.

## ⏱ Posição no tempo

| | |
|---|---|
| Reunião | **19 de 34** de Luiza Barcelos nos acervos |
| Data | **2025-06-20** — **462 dias atrás** |
| Depois dela | **15 reuniões** — a última em **2026-09-22** |
| Status no corpus | `Ongoing` — varredura de **2026-09-21**, que é a data da **leitura**, não da transição |

⚠ **Esta reunião é história.** O que ela afirma **aconteceu em 2025-06-20**; o que prometia
**estava por vir naquela data**, e o cumprimento **não está verificado** — há **15 reuniões posteriores** onde conferir.

## ⚠ Fatos propostos — ainda NÃO são fatos

### ✅ Aconteceu — em 2025-06-20

- marco: Michele Rodrigues mencionou que a mudança seria interessante para a virada do inverno. — [resumo Gemini 2025-06-20 · 00:07:13] ⚠ PROPOSTA · DERIVADA · conferível na transcrição · 🟢 CONFIRMADA NA FALA (2026-09-26 · Michelle Rodrigues · 00:07:13)

## Governança

### Quem pode alterar este documento
🔴 **Ninguém.** É registro de entrada. Aprovar ou recusar uma proposta gera
registro novo no destino, apontando para este.
